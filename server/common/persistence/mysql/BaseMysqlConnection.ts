...
import { retryMysqlStartupQuery } from './mysql-readiness.ts';
import { createMysqlMigrator } from '../migrations/createMysqlMigrator.ts';

export type MysqlEnv = {
    MYSQL_HOST?: string;
    MYSQL_HOST_FILE?: string;
    MYSQL_USER?: string;
    MYSQL_USER_FILE?: string;
    MYSQL_PASSWORD?: string;
    MYSQL_PASSWORD_FILE?: string;
    MYSQL_DB?: string;
    MYSQL_DB_FILE?: string;
};

export class BaseMysqlConnection {
    private pool?: Pool;

    constructor(
        private readonly env: MysqlEnv,
        private readonly migrations: RunnableMigration<Pool>[],
    ) {}
    ...
    async init(): Promise<void> {
        ...
        const migrator = createMysqlMigrator(this.pool!, this.migrations);
        await migrator.up();
