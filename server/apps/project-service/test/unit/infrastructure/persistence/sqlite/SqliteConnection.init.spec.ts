...
describe('SqliteConnection init', () => {
    ...
    test('creates missing directory, opens database, and initializes schema', async () => {
        const connection = new SqliteConnection('/tmp/project/project.sqlite');

        await connection.init();

        expect(fsMock.existsSync).toHaveBeenCalledWith('/tmp/project');
        expect(fsMock.mkdirSync).toHaveBeenCalledWith('/tmp/project', {
            recursive: true,
        });
        expect(databaseInstances[0].location).toBe(
            '/tmp/project/project.sqlite',
        );
