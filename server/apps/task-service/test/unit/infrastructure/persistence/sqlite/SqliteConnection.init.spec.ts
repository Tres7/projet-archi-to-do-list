...
describe('SqliteConnection init', () => {
    ...
    test('creates missing directory, opens database, and initializes schema', async () => {
        const connection = new SqliteConnection('/tmp/task/task.sqlite');

        await connection.init();

        expect(fsMock.existsSync).toHaveBeenCalledWith('/tmp/task');
        expect(fsMock.mkdirSync).toHaveBeenCalledWith('/tmp/task', {
            recursive: true,
        });
        expect(databaseInstances[0].location).toBe('/tmp/task/task.sqlite');
