...
describe('InMemoryConnection', () => {
    test('requires init before using tables', () => {
        ...
        expect(() => connection.table('tasks')).toThrow(
    ...
    test('creates, clears, and tears down tables', async () => {
        ...
        connection.table<{ id: string }>('tasks').set('1', { id: '1' });

        expect(connection.table('tasks').size).toBe(1);

        await connection.clearDatabase();
        expect(connection.table('tasks').size).toBe(0);
