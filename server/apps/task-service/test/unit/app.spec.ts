...
import { FakeTaskRepository } from './helpers/FakeTaskRepository.ts';
...
const busMock = {
    ...
    respond: jest.fn<MessageBus['respond']>(),
...
describe('createApp', () => {
    ...
    test('creates app, registers consumers, and exposes health endpoint', async () => {
        ...
        expect(busMock.subscribe).toHaveBeenCalledTimes(3);
        expect(busMock.respond).toHaveBeenCalledTimes(1);
        expect(response.statusCode).toBe(200);
        expect(response.jsonBody).toEqual({ ok: true });
    });

    test('starts and stops task modules', async () => {
        const container: PersistenceContainer = {
            connection: new ConnectionStub(),
            repositories: {
                taskRepository: new FakeTaskRepository(),
        ...
        expect(busMock.start).toHaveBeenCalledWith('task-service');
        expect(busMock.stop).toHaveBeenCalledWith('task-service');
